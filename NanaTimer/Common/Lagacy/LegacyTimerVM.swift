...
final class LegacyTimerVM {
    ...
    var daysText: LocalizedStringKey{
        if timerModel.state == .finished && timerModel.upDays <= 7{
    ...
    func start() {
        ...
        timerModel.updateState(to: .running)
    }
    ...
    func reset() {
        TimerService.shared.invalidate()
        timerModel.updateState(to: .ready)
        timerModel.upTime = 0
    ...
    private func end() {
        TimerService.shared.invalidate()
        timerModel.updateState(to: .finished)
        timerModel.upTime = timerModel.settedTime
    }
    
    private func background() {
        TimerService.shared.invalidate()
        timerModel.updateState(to: .backgrounded)
    }
